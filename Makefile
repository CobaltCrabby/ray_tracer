...
bin/raytracer: $(patsubst %.cpp, %.o, $(wildcard src/*.cpp))
	clang++ $^ third_party/vkbootstrap/VkBootstrap.cpp -o $@ -std=c++17 -arch arm64 -Wno-nullability-completeness -L/opt/homebrew/Cellar/sdl2/2.30.5/lib -lSDL2 -lSDL2main -Wl,-rpath,/opt/homebrew/Cellar/sdl2/2.30.5/lib -Wl,-rpath,/usr/local/lib third_party/libtinyobjloader.a third_party/libimgui.a /usr/local/lib/libvulkan.dylib
